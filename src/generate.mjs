...
const HOOK_MENU = [
  ...
  // ↓ 2026-08-20 集客特化アカウント分析（_private/threads-research-toam-marke.md）より追加
  //   シグネチャー口癖は2枠入れて出現率を上げている（意図的な重み付け）
  '対象の名指し＋限定数の予告（例「AI副業で最初の1円が遠い人 やることは2つだけ」）答えは1行目に書かない',
  `シグネチャー口癖で始める（「${SIGNATURE}」に続けて持論を強く断定する 例「${SIGNATURE} 副業が続かないのは意志の問題じゃない」）`,
  `シグネチャー口癖で始める（「${SIGNATURE}」に続けて持論を強く断定する 例「${SIGNATURE} 道具を増やすほど稼げなくなる」）`,
  '常識の否定＋命令（例「スキルがついてから始めるは嘘です 先に始めてください」）',
  '悩みの名指し＋数字の約束（例「作業が終わらない人 この3つをやめると平日1時間浮きます」）',
];
...
const CLOSING_MENU = {
  ...
  action:
    '最後は「今日はこれだけやってみて」と具体的なアクションを1つ置き 「5分で終わります」のように所要時間かハードルの低さを1言そえて締める 質問は付けない',
  assert:
...
- 例外: シグネチャー口癖「${SIGNATURE}」と固定実績（約半年で月+5万）は繰り返し使ってよい 決まった反復はキャラの記憶フックになる`;
...
- 感情や悩みを説明で終わらせない 読者が昨日経験したような場面（いつ 誰と 何をあきらめたか）まで具体化する 例「時間がない」→「子どもを寝かせたあと 机に向かう気力が残ってない」
...
- 感情や悩みは説明で終わらせず 読者が昨日経験したような場面まで具体化する（例「時間がない」→「夜ごはんのあと スマホを見て1日が終わる」）
- 前回までと同じ言い回し・数字・型の使い回しはしない 毎回ちがう切り口にする
...
// --- ツリーの構成メニュー（ノウハウツリー用・毎回1つ乱択）---
// front: 1本目に価値の8割を入れる保存狙い型（2026-08調査: 日本語Threadsの主流）
// cliffhanger: 親はフックだけ→2本目の最後を文の途中で切る→最終本で回収する型
//   （2026-08-20 集客特化アカ分析より。文の途中切りが「投稿クリック率」「滞在時間」の
//    Meta公式シグナルに効く。詳細は _private/threads-research-toam-marke.md）
// ※検品の出し惜しみNG（「続きは次」等）に引っかからないよう「予告を書かずただ文を切る」と指示する
const THREAD_FORMAT_MENU = [
  {
    key: 'front',
...
- 基本は2本 深掘りに値する内容があるときだけ3本 無理に引き伸ばさない`,
  ...
  {
    key: 'cliffhanger',
    rule: `【ツリーの型（3本・クリフハンガー型）】
- 1本目: フックだけを1〜3行で書く 本文や答えはまだ書かない 「やめたことは3つだけです」のように中身の数だけ予告して止める
- 2本目: 「先に結論から言うと」で結論を言い切ってから __FIRST__の体験・数字・具体場面で展開する ①②③の箇条書きを使ってよい ★最後の行は「なんでかというと」「変えたのは1つだけで」のように文の途中で切って終える（「続きは次で」のような予告文は書かない ただ文が切れたまま終える）
- 3本目: 切れた文の続きから自然に書き出す → 理由と学びを言い切る → __CLOSING__

【本数の決め方】
- この型は必ず3本 3本目が2本目の続きとしてつながって読めることを最優先にする`,
  },
];
// テスト用に THREAD_FORMAT=front|cliffhanger で固定できる（普段は毎回乱択）
const threadFormat =
  THREAD_FORMAT_MENU.find((f) => f.key === process.env.THREAD_FORMAT) ||
  rand(THREAD_FORMAT_MENU);
...
if (article) {
  ...
  console.log(
    `🧵 ノウハウ枠：長文ツリーを生成します → ${topic}（構成=${threadFormat.key}）`,
  );
  const parts = await withThreadValidation(
