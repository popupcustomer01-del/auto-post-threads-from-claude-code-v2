...
// closingKey: 枠の締め方(question=問いで終わることを要求 lp=最終行にプロフ案内を要求 それ以外は問い禁止)
...
export function validatePost(
  ...
  // LP誘導締め(closing=lp 2026-08-20 LP送客調査より)の最終本だけ 最終行の
  // 「プロフィールのリンクへ」の一行お知らせを許可する(それ以外の出し惜しみは従来どおりNG)
  const isLpClosing = closingKey === 'lp' && isLastPart;
  ...
  const teaserScope = isLpClosing ? lines.slice(0, -1).join('\n') : text;
  if (/知りたくない[?？]|知りたい人は|続きは(プロフ|リプ|次)|詳しくはプロフ/.test(teaserScope))
    problems.push('中身を隠して引っ張る出し惜しみ表現がある');
  if (isLpClosing && !/プロフ/.test(lastLine))
    problems.push(
      'LP誘導枠なのに最終行が「プロフィールのリンクにまとめてます」等の一行お知らせになっていない',
    );
